...
dev = tvm.cpu() if target == "llvm" else tvm.device("cuda", 0)
# If true, then a sparse variant of the network will be run and
# benchmarked.
measure_sparse = False
...
class ModelInfo:
    def __init__(self, name, shape2input_fn):
        self.name = name
        self.shape2input_fn = shape2input_fn

def shape2input_fn_v1(shape_dict):
    return {0: shape_dict["input_1"]}

model_info = {
    "prunebert_base_uncased": ModelInfo(
...
def import_graphdef(
    ...
    abs_path = os.path.abspath(".workspace/tf_pb")
...
def wrap_import_graphdef(
    ...
    mod, params, shape_dict = import_graphdef(
            model_info[short_name].name,
            batch_size,
            seq_len,
            save_relay=save_relay,
            relay_file=relay_file,
            relay_params=relay_params,
        )
    input_dict = model_info[short_name].shape2input_fn(shape_dict)
