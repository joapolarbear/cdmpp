...
def get_min_latency(results):
    min_latency = INVALID_TIME_UPPER
    for res in results:
        dur, std = parse_cost(res)
        if dur is None:
            continue
        min_latency = min(min_latency, dur)
    return min_latency
...
class Dataset:
    ...
    def update_from_measure_pairs(self, inputs: List[MeasureInput], results: List[MeasureResult]):
        ...
        for task, (inputs, results) in new_data.items():
            ...
            self.load_task_data(task, features, normalized_throughputs, min_latency)
    ...
    for filename in tqdm(log_files):
        ...
        if os.path.exists(cache_file):
            ...
            for task, (inputs, results) in measure_records.items():
                features_, normalized_throughputs, task_ids =\
                    get_per_store_features_from_measure_pairs(inputs, results)

                assert not np.any(task_ids)   # all task ids should be zero
                min_latency_ = get_min_latency(results)
                if min_latency_ == INVALID_TIME_UPPER:
                    # no valid records
                    continue
                # else:
